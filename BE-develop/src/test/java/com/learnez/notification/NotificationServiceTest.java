...
public class NotificationServiceTest {
    ...
    // TC-NOTI-006: updateNoti when NotificationReceive missing should cause clear
    // handling (spec)
    @Test
    public void TC_NOTI_006_updateNoti_missingReceiver_shouldThrowOrHandle() {
        // Arrange
        HttpServletRequest request = mock(HttpServletRequest.class);
        ...
        Long missingNotiId = 2001L;
        when(notificationReceiveRepository.findByNotificationIdAndReceiverId(missingNotiId, 44L)).thenReturn(null);

        // Act & Assert
        // Business spec: service should handle missing NotificationReceive gracefully
        // (e.g., throw WebToeicException).
        // Current implementation will cause NPE; this test documents the expected
        // behavior and may fail.
        assertThrows(Exception.class, () -> notiService.updateNoti(request, List.of(missingNotiId)));
    }

    // TC-NOTI-007: countNoti - user not found should throw NOT_EXISTED
    @Test
    public void TC_NOTI_007_countNoti_userNotFound_shouldThrow() {
        // Arrange
        HttpServletRequest request = mock(HttpServletRequest.class);
        ...
        // Act & Assert
        assertThrows(Exception.class, () -> notiService.countNoti(request));
        verify(notiRepository, never()).countNotiByReceiverId(any());
        // Rollback: mock repos
    }

    // TC-NOTI-008: listNoti - user not found should throw
    @Test
    public void TC_NOTI_008_listNoti_userNotFound_shouldThrow() {
        // Arrange
        HttpServletRequest request = mock(HttpServletRequest.class);
        String nonExistEmail = "nonexist2@example.com";
        when(jwtUtil.getEmailFromToken(request)).thenReturn(nonExistEmail);
        when(userRepository.findByEmail(nonExistEmail)).thenReturn(Optional.empty());

        // Act & Assert
        assertThrows(Exception.class, () -> notiService.listNoti(request, Pageable.unpaged()));
        verify(notiRepository, never()).filter(any(), any());
        // Rollback: mock repos
    }

    // TC-NOTI-009: updateNoti - user not found should throw
    @Test
    public void TC_NOTI_009_updateNoti_userNotFound_shouldThrow() {
        // Arrange
        HttpServletRequest request = mock(HttpServletRequest.class);
        String nonExistEmail = "nonexist3@example.com";
        when(jwtUtil.getEmailFromToken(request)).thenReturn(nonExistEmail);
        when(userRepository.findByEmail(nonExistEmail)).thenReturn(Optional.empty());

        // Act & Assert
        assertThrows(Exception.class, () -> notiService.updateNoti(request, List.of(1L)));
